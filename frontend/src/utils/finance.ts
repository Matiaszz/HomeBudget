...
export function recalculateSummary(txs: Transaction[]): FamilyBudgetSummaryDto {
  ...
  txs.forEach((t) => {
    ...
    if (t.familiarId) {
      ...
      if (t.type === "expense") {
        const currentExp = familiarExpensesMap.get(fid) || { name: fname, total: 0 };
        familiarExpensesMap.set(fid, { name: fname, total: currentExp.total + amount });
      }

      // 2. Agrupamento do resumo de saldos (entrada/saída) por membro
      const currentSum = familiarSummariesMap.get(fid) || { name: fname, income: 0, expense: 0 };
      if (t.type === "income") {
